...
import torch
import sys
sys.path.append("/data/samyakp/InternVideo/InternVideo2/multi_modality/utils")
...
import sys
sys.path.insert(0, "/data/samyakp/InternVideo/InternVideo2")
sys.path.insert(0, "/data/samyakp/InternVideo/InternVideo2/multi_modality")
...
for data_file in data_files:
    ...
    # OUTPUT_DIR = f"/data/samyakp/llava_video_data/eval_videos/{ds_name}/internvideo2_patch_embeddings"
    OUTPUT_DIR = f"/data/samyakp/llava_video_data/eval_videos/{ds_name}/internvideo2_global_embeddings"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    EXISTING_EMBEDDINGS = set(os.path.join(OUTPUT_DIR, fname) for fname in os.listdir(OUTPUT_DIR))
    #new_json = data_file.split(".json")[0] + "_with_internVideo2_patch_embeddings.json"
    new_json = data_file.split(".json")[0] + "_with_internVideo2_global_embeddings.json"
    ...
    for rec in tqdm(records):
        ...
        if internvideo2_embed_path not in EXISTING_EMBEDDINGS:
            ...
            with torch.no_grad():
                ...
                _, vid_emb = model.encode_vision(vid_t, test=True) #global

            vid_emb = vid_emb.squeeze(0).cpu() 

            torch.save(vid_emb, internvideo2_embed_path)

        #rec["internVideo2_patch_embedding"] = internvideo2_embed_path
        rec["internVideo2_global_embedding"] = internvideo2_embed_path
